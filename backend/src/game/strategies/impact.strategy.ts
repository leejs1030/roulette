...
import { ImpactSkillEffect, SkillType } from 'common';
...
export class ImpactSkillStrategy implements SkillStrategy<SkillType.Impact> {
  ...
  execute(room: GameRoom, skillPosition: SkillPosition): void {
    this.logger.log(
      `Room ${room.id}: Impact skill used at (${skillPosition.x}, ${skillPosition.y}) with radius ${IMPACT_SKILL_RADIUS} and force ${IMPACT_SKILL_FORCE}`,
    );
