...
export class ManagerOnlyGuard implements CanActivate {
  ...
  private toNumber(value: string | number): number {
    ...
    if (isNaN(num)) {
      throw new WsException('유효하지 않은 숫자입니다.');
    }
    return num;
  }
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const roomId = this.toNumber(data?.roomId);
    ...
    if (!data || typeof roomId !== 'number') {
      this.logger.warn(
        `ManagerOnlyGuard: RoomId not provided or invalid in request data from ${client.id}. Denying access.`,
      );
    ...
    try {
      ...
      if (!isManager) {
        this.logger.log(
          `ManagerOnlyGuard: User ${user.nickname} (${user.id}) is not a manager of room ${roomId}. Denying access.`,
        );
        throw new WsException('방의 매니저만 이 작업을 수행할 수 있습니다.');
      }
      this.logger.log(
        `ManagerOnlyGuard: User ${user.nickname} (${user.id}) is a manager of room ${roomId}. Granting access.`,
      );
      return true;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `ManagerOnlyGuard: Error checking manager status for user ${user.nickname} (${user.id}) in room ${roomId}: ${message}`,
      );
